...
export default async function HomePage() {
  ...
  if (!surahs || surahs.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-black text-white font-sans">
        ...
        <Link href="/" className="px-6 py-2 bg-emerald-500 text-black font-bold rounded-xl hover:bg-emerald-400 transition-colors">
          Try Again
        </Link>
      </div>
    );
  ...
  return (
    <main className="max-w-6xl mx-auto p-6 min-h-screen bg-black font-sans">
      {/* Header */}
          ...
          <Link key={surah.id} href={`/surah/${surah.id}`}>
            <div className="group p-6 bg-zinc-900/40 border border-zinc-800/50 rounded-2xl hover:border-emerald-500/40 hover:bg-zinc-800/40 transition-all duration-300 cursor-pointer relative overflow-hidden">
              ...
              <div className="flex justify-between items-start mb-4 relative z-10">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex flex-col mt-0.5">
                      {/* ✅ Bengali Name added here */}
                      <p className="text-[13px] text-emerald-500 font-medium leading-tight">
                        {surah.bnName}
                      </p>
                ...
                {/* Arabic Name */}
                <span className="text-3xl text-emerald-500 font-arabic leading-none">
                  {surah.name}
                </span>
              </div>
